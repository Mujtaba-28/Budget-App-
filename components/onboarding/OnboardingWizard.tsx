...
export const OnboardingWizard: React.FC = () => {
    ...
    const handleLaunch = () => {
        ...
        setTimeout(() => {
            // PASS true to clearData if 'Start Fresh' selected.
            // This wipes all contexts and creates a fresh "Personal" one with the given budget.
        ...
        <div className="fixed inset-0 z-[200] bg-gradient-to-br from-emerald-50 via-teal-50 to-emerald-100 dark:from-[#021c17] dark:via-[#032b24] dark:to-[#021c17] flex items-center justify-center p-6 animate-in fade-in duration-500">
            <div className="w-full max-w-sm relative flex flex-col h-full max-h-[700px] justify-center">
                ...
                {step > 1 && (
                    <button 
                        onClick={handleBack}
                        className="absolute top-0 left-0 p-3 rounded-full hover:bg-white/20 text-slate-500 dark:text-emerald-100 transition-colors z-50 active:scale-90"
                    >
                        <ArrowLeft size={24} />
                    </button>
                )}
                    ...
                    <div className="text-center animate-in zoom-in duration-700 flex flex-col items-center">
                        ...
                        <div className="h-8 mb-12 overflow-hidden relative w-full">
                            {taglines.map((tag, i) => (
                                <p 
                                    key={i} 
                                    className={`absolute w-full text-lg font-bold text-slate-500 dark:text-emerald-200/80 transition-all duration-500 transform ${
                                        i === taglineIndex ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
                                    }`}
                                >
                                    {tag}
                                </p>
                        ...
                        <button onClick={handleNext} className="w-full py-4 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-2xl font-bold text-lg shadow-lg shadow-emerald-600/30 active:scale-95 transition-all hover:shadow-xl flex items-center justify-center gap-2 group">
                            Get Started <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform"/>
                        </button>
                    </div>
                )}

                {/* STEP 2: IMMERSIVE TOUR */}
                {step === 2 && (
                            ...
                            <div className="flex justify-center gap-3 mb-6">
                                {tourSteps.map((_, i) => (
                                    <button 
                                        ...
                                        className={`h-2 rounded-full transition-all duration-500 ease-out ${i === tourIndex ? 'w-8 bg-emerald-500 shadow-md' : 'w-2 bg-slate-300 dark:bg-slate-700 hover:bg-emerald-200'}`}
                                    />
                                ))}
                            </div>
                ...
                {step === 3 && (
                    <div className="bg-white/80 dark:bg-[#0a3831]/80 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-2xl shadow-emerald-900/10 border border-white/40 dark:border-emerald-500/10 animate-in slide-in-from-right-8 duration-500">
                        <div className="text-center mb-8">
                            ...
                            <p className="text-slate-400 text-sm mt-1">Let's verify your identity</p>
                        </div>
                        
                        <div className="space-y-6">
                            <div className="group">
                                {/* Live Greeting Label */}
                                <label className="block text-xs font-extrabold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest mb-3 ml-1 transition-all">
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        className="w-full bg-slate-50 dark:bg-black/20 p-4 pl-12 rounded-2xl font-bold text-lg outline-none text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500/30 focus:bg-white dark:focus:bg-black/40 transition-all placeholder:text-slate-300 placeholder:font-normal"
                                        autoFocus
                                    />
                                </div>
                            </div>
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    {currencies.map(c => (
                                        <button 
                                            ...
                                            className={`p-4 rounded-2xl font-bold text-xl transition-all duration-300 ${currency === c ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20 scale-105 ring-2 ring-emerald-600 ring-offset-2 ring-offset-white dark:ring-offset-[#0a3831]' : 'bg-slate-50 dark:bg-black/20 text-slate-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 hover:text-emerald-600'}`}
                                        >
                                            {c}
                                        </button>
                                    ))}
                        ...
                        <button 
                            onClick={handleNext} 
                            disabled={!name.trim()} 
                            className="w-full mt-8 py-4 bg-emerald-950 dark:bg-emerald-100 text-white dark:text-emerald-950 rounded-2xl font-bold disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-xl transition-all active:scale-[0.98]"
                        >
                            Continue
                        </button>
                    </div>
                ...
                {step === 4 && (
                    <div className="bg-white/80 dark:bg-[#0a3831]/80 backdrop-blur-xl p-6 rounded-[2.5rem] shadow-2xl shadow-emerald-900/10 border border-white/40 dark:border-emerald-500/10 animate-in slide-in-from-right-8 duration-500 overflow-y-auto max-h-[600px] scrollbar-hide">
                        <div className="text-center mb-6">
                            <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Setup Data</h2>
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-6">
                            <button 
                                ...
                                className={`p-4 rounded-[2rem] flex flex-col items-center justify-center gap-2 transition-all duration-300 border-2 ${shouldWipe ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-500 text-emerald-700 dark:text-emerald-300 shadow-md' : 'bg-slate-50 dark:bg-black/20 border-transparent text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
                            >
                                <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-colors ${shouldWipe ? 'border-emerald-500 bg-emerald-500 text-white' : 'border-slate-300 text-transparent'}`}>
                            ...
                            <button 
                                ...
                                className={`p-4 rounded-[2rem] flex flex-col items-center justify-center gap-2 transition-all duration-300 border-2 ${!shouldWipe ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-500 text-indigo-700 dark:text-indigo-300 shadow-md' : 'bg-slate-50 dark:bg-black/20 border-transparent text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
                            >
                                <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-colors ${!shouldWipe ? 'border-indigo-500 bg-indigo-500 text-white' : 'border-slate-300 text-transparent'}`}>
                                    <Check size={14} strokeWidth={4}/>
                                </div>
                        ...
                        {shouldWipe ? (
                            <div className="animate-in slide-in-from-bottom-2 fade-in">
                                ...
                                <div className="relative mb-4">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-slate-50 dark:bg-black/20 p-4 pl-10 rounded-2xl font-black text-3xl outline-none text-emerald-600 dark:text-emerald-400 focus:ring-2 focus:ring-emerald-500/30 transition-all placeholder:text-slate-200"
                                        autoFocus
                                    />
                                </div>
                                ...
                                <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-2 mb-2">
                                    {quickBudgets.map(amt => (
                                        <button 
                                            ...
                                            className="px-3 py-2 bg-slate-100 dark:bg-black/30 rounded-xl text-xs font-bold text-slate-500 hover:bg-emerald-100 hover:text-emerald-600 transition-colors whitespace-nowrap"
                                        >
                            ...
                            <div className="bg-indigo-50 dark:bg-indigo-900/10 p-4 rounded-2xl mb-4 border border-indigo-100 dark:border-indigo-800/30 animate-in slide-in-from-bottom-2">
                                <div className="flex gap-3">
                                    <div className="p-2 bg-indigo-100 dark:bg-indigo-900/50 rounded-xl text-indigo-600 h-fit">
                                        <LayoutGrid size={20}/>
                                    </div>
                                    <div>
                                        ...
                                        <p className="text-xs text-indigo-700/70 dark:text-indigo-300/70 mt-1 leading-relaxed">
                                            We'll load sample transactions, goals, and budgets so you can explore the app's features immediately.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <button 
                            onClick={handleNext} 
                            disabled={shouldWipe && !budget}
                            className="w-full mt-2 py-4 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-2xl font-bold text-lg shadow-lg shadow-emerald-600/30 hover:shadow-xl active:scale-[0.98] transition-all flex items-center justify-center gap-2 group disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            Launch App <PartyPopper size={20} className="group-hover:-rotate-12 transition-transform"/>
                        </button>
                    </div>
                )}
